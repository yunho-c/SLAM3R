    ...
    def __getitem__(self, idx):
        ...
        for v, view in enumerate(views):
            ...
            if self.need_pts3d:
                ...
                pts3d, pts3d_cam, valid_mask = depthmap_to_world_and_camera_coordinates(**view)
                view['pts3d_cam'] = pts3d_cam  #(H, W, 3)
                view['pts3d'] = pts3d 
                view['valid_mask'] = valid_mask & np.isfinite(pts3d).all(axis=-1)
                view['inv_camera_pose'] = np.linalg.inv(view['camera_pose'])
            else:
                view['pts3d'] = np.full((height, width, 3), np.nan, dtype=np.float32)
                if 'depthmap' in view:
                    view['valid_mask'] = view['depthmap'] > 0
                else:
                    view['valid_mask'] = np.full((height, width), False, dtype=bool)
                
            if self.depth_img:    
            #transform depthmap to [-1,1]
                depthmap_img = view['depthmap']
                depthmap_img[~view['valid_mask']] = 0
                max_depth = min(np.max(view['depthmap']), 20)
                depthmap_img = np.clip(depthmap_img, 0, max_depth) / max_depth  #from 0 to 1
                depthmap_img = depthmap_img * 2 - 1  #from -1 to 1
                view['depthmap_img'] = np.repeat(depthmap_img[:,:,None], 3, axis=2)
            
            if self.pointmap_img:
                pointmap_img = view['pts3d_cam']
                #add noise to pointmap
                mask = view['valid_mask']
                scale = np.linalg.norm(pointmap_img[mask], axis=-1).mean()
                # print(scale)
                if self.add_noise:
                    #尺度高斯噪声
                    np.random.seed(idx)
                    scale_noise = np.random.normal(1, 0.05, pointmap_img.shape[:2])
                    scale_noise = np.clip(scale_noise, 0.5, 1.5)
                    scale = scale * scale_noise[:, :, None]
                view['pointmap_img'] = pointmap_img / scale
                view['pointmap_img'] = torch.tensor(view['pointmap_img'], dtype=view['img'].dtype).permute(2,0,1) 
        ...
        for view in views:
            ...
            # transpose_to_landscape(view, force=True if self.transpose else False)
